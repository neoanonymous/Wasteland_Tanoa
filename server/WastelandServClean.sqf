...
#define CLEANUP_INTERVAL (10) // Interval to run the cleanup
#define ITEM_CLEANUP_TIME (60) // Dropped player items cleanup time
#define MONEY_CLEANUP_TIME (60*60) // Dropped money cleanup time
#define STORE_CLEANUP_TIME (8*60) // Dropped store items cleanup time
#define STORE_CLEANUP_RADIUS 75 // Radius in meters to scan near store NPCs for dropped items
...
#define GROUP_CLEANUP_TIME (10) // How long a group must have been empty before deleting it
...
_storeNPCs = allUnits select {[["GenStore","GunStore","VehStore","walmart"], vehicleVarName _x] call fn_startsWith};
_baseClass = "";
